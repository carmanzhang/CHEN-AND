"""
对于合成的SinoPubMed数据库，使用GPU每一次对聚类参数调整会生成一个全量消歧文件，这个脚本的目的是为了评估 每个聚类参数对应的消歧结果
"""
import glob
import os
import sys
...
if len(aid_files) == 0:
    aid_files = sorted(glob.glob(os.path.join(cached_dir, 'sinopubmed-and-result-1670916318-*')))
...
sql_block = r'''
select xmol_clean_author_name,
       arrayMap(x->x[1], groupArray([pid_ao, xmol_aid]) as tmp_arr) as pid_aos,
    ...
    with open(f, 'r') as fr:
        for line in fr:
            ...
            try:
                ...
                print(e)
                break
    aid_systems.append(and_param_spec)
...
def combine_existing_and_our_author_ids(input):
    our_aids = [input[k] for k in aid_systems]
    our_aids = np.array(our_aids)
    our_aids = our_aids.transpose()
    return our_aids


df_block_test['combining_author_ids'] = df_block_test[aid_systems].apply(
    lambda x: combine_existing_and_our_author_ids(x), axis=1)
for c in aid_systems:
    del df_block_test[c]

l = len(df_pairwise_test)
print('df_pairwise_test.shape: ', )
df_pairwise_test = df_pairwise_test[
    df_pairwise_test[aid_systems].apply(lambda x: len([n for n in x if n == -1]), axis=1) == 0]
...
all_aid_systems = np.array(aid_systems)
all_metrics = []
...
for id_sys_name, sub_df in sub_dfs:
    ...
    metric_str = '\t'.join(
        ['MFTD-' + id_sys_name, str(num_pairwise_instances) + '_' + str(num_block_instances)]
        + [str(round(n * 100, 2)) for n in list(metrics_prf1 + metrics_micro_b3 + list(metrics_macro_b3))])
    print('\t'.join(metric_names + ['pP', 'pR', 'pF', 'bP', 'bR', 'bF', 'Macro-bP', 'Macro-bR', 'Macro-bF']))
    print(metric_str)
    save_result(spec='CHENAND-Online.txt', metrics=metric_str)
