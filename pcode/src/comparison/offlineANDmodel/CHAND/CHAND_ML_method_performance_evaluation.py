'''
using available metadata to develop disambiguation model for SinoMed Chinese authors
'''
import os
...
sql = r'''
select xmol_clean_author_name, concat(toString(new_pid1), '_', toString(author_position1)) as pid_ao1, concat(toString(new_pid2), '_', toString(author_position2)) as pid_ao2, xmol_aid1, xmol_raw_citation1, coauthors1, author_affiliations1, author_provinces1, author_cities1, author_postcodes1, en_infersent_embedding1, en_sent2vec_embedding1, en_specter_embedding1, zh_medbert_embedding1, jd1, st1, title1, journal_title1, pub_year1, xmol_aid2, xmol_raw_citation2, coauthors2, author_affiliations2, author_provinces2, author_cities2, author_postcodes2, en_infersent_embedding2, en_sent2vec_embedding2, en_specter_embedding2, zh_medbert_embedding2, jd2, st2, title2, journal_title2, pub_year2, commonness, train1_test0_val2
from and_ds_ench.CHENAND_sub_CH_dataset_sampled_author_pair ANY
         inner join (select pid_ao as pid_ao1, toInt32(round(log2(block_size))) as commonness
                     from and_ds_ench.sinomed_ambiguous_author_with_nameblocking_zh) using pid_ao1
;
'''
...
for i, row in tqdm(df.iterrows(), total=len(df)):
    xmol_clean_author_name, pid_ao1, pid_ao2, xmol_aid1, xmol_raw_citation1, \
    coauthors1, author_affiliations1, author_provinces1, author_cities1, author_postcodes1, \
    en_infersent_embedding1, en_sent2vec_embedding1, en_specter_embedding1, zh_medbert_embedding1, jd1, st1, \
    ...
    en_infersent_embedding2, en_sent2vec_embedding2, en_specter_embedding2, zh_medbert_embedding2, jd2, st2, \
    ...
    sim_country_intersection = 1  # note 对于 SinoMed中的作者，国家都是“中国”
...
feature_group_dict = {
    # 'CA': ['f1'],  # coauthors
    # 'AF': ['f2'],  # original_affiliations
    # 'NT-PV-CT-PC': ['f3', 'f4', 'f5', 'f6'],  # country + province + city + postcode
    'CM-PY-JD-ST': ['f0', 'f15', 'f7', 'f8'],  # commonness + pub_year + jd + st
    ...
    'CA⊕AF⊕NT-PV-CT-PC⊕CM-PY-JD-ST⊕SPECTER': ['f1'] + ['f2'] + ['f3', 'f4', 'f5', 'f6'] + ['f0', 'f15', 'f7', 'f8'] + ['f11'],
    'CA⊕AF⊕NT-PV-CT-PC⊕CM-PY-JD-ST⊕InferSent': ['f1'] + ['f2'] + ['f3', 'f4', 'f5', 'f6'] + ['f0', 'f15', 'f7', 'f8'] + ['f9'],
    'CA⊕AF⊕NT-PV-CT-PC⊕CM-PY-JD-ST⊕Sent2Vec': ['f1'] + ['f2'] + ['f3', 'f4', 'f5', 'f6'] + ['f0', 'f15', 'f7', 'f8'] + ['f10'],
    'CA⊕AF⊕NT-PV-CT-PC⊕CM-PY-JD-ST⊕MedBERT': ['f1'] + ['f2'] + ['f3', 'f4', 'f5', 'f6'] + ['f0', 'f15', 'f7', 'f8'] + ['f12'],
    'CA⊕AF⊕NT-PV-CT-PC⊕CM-PY-JD-ST⊕TitleSim': ['f1'] + ['f2'] + ['f3', 'f4', 'f5', 'f6'] + ['f0', 'f15', 'f7', 'f8'] + ['f13'],
    'CA⊕AF⊕NT-PV-CT-PC⊕CM-PY-JD-ST⊕SPECTER⊕JT': ['f1'] + ['f2'] + ['f3', 'f4', 'f5', 'f6'] + ['f0', 'f15', 'f7', 'f8'] + ['f13']
                                                + ['f14']
    # Note ##########################################################
    # Note GPU对SinoMed消歧使用的特征如下：
    # all_features = torch.cat((commonness_feature, pub_year_feature, embedding_similarity_feature.unsqueeze(1), entity_features), dim=1)
    # arrayMap(x->concat('jd', '_', x), jd),
    # arrayMap(x->concat('st', '_', x), st),
    # arrayMap(x->concat('ca', '_', x), coauthor_list),
    # arrayMap(x->concat('oa', '_', x), original_aff),
...
sql = r'''
select xmol_clean_author_name, concat(toString(new_pid), '_', toString(author_position)) as pid_ao, xmol_aid, new_pid, xmol_raw_citation, commonness, coauthors, author_affiliations, author_provinces, author_cities, author_postcodes, en_infersent_embedding, en_sent2vec_embedding, en_specter_embedding, zh_medbert_embedding, title, jd, st, journal_title, pub_year
from and_ds_ench.CH_EN_AND_dataset ANY
         inner join (select pid_ao, toInt32(round(log2(block_size))) as commonness
                     from and_ds_ench.sinomed_ambiguous_author_with_nameblocking_zh) using pid_ao
-- Note denoting Chinese AND dataset
where CHAND > 0 and train1_test0_val2 in (2)
...
def calc_groundtruth_predictions_given_dataset(df, model, feature_selector):
    """
    ...
    for block_name, sub_df in tqdm(df.groupby(['xmol_clean_author_name'])):
        ...
        block_author_feature_and_metadata_2d = np.zeros(shape=(num_block_authors, num_block_authors), dtype=np.float)

        for i in range(0, num_block_authors - 1, 1):
            for j in range(i + 1, num_block_authors, 1):
                xmol_clean_author_name1, pid_ao1, xmol_aid1, pid1, xmol_raw_citation1, commonness1, \
                coauthors1, author_affiliations1, author_provinces1, author_cities1, author_postcodes1, \
                ...
                xmol_clean_author_name2, pid_ao2, xmol_aid2, pid2, xmol_raw_citation2, commonness2, \
                coauthors2, author_affiliations2, author_provinces2, author_cities2, author_postcodes2, \
                ...
                a2a_author_feature_vec = np.array([commonness1,  # 0
                                                   sim_coauthors_intersection,  # 1
                                                   sim_affiliations_intersection,  # 2
                                                   sim_country_intersection,  # 3
                                                   sim_provinces_intersection,  # 4
                                                   ...
                                                   sim_journal_jaccard,  # 14
                                                   sim_pubyear_diff  # 15
                                                   ]).astype(np.float)[feature_selector]

                a2a_distance = 1 - model.predict([a2a_author_feature_vec])[0]
                block_author_feature_and_metadata_2d[i][j] = a2a_distance

        # Note convert author-author 2D array to 2D symmetric matrix
        block_author_feature_and_metadata_2d = (block_author_feature_and_metadata_2d + block_author_feature_and_metadata_2d.T)

        feature_and_metadata_ds.append(block_author_feature_and_metadata_2d)
...
for feature_group_as_a_method, feature_group in feature_group_dict.items():
    ...
    ground_truths_ds, feature_and_metadata_ds = calc_groundtruth_predictions_given_dataset(df_dev, model, feature_selector)
...
from and_ds_ench.CH_EN_AND_dataset ANY
         inner join (select pid_ao, toInt32(round(log2(block_size))) as commonness
                     from and_ds_ench.sinomed_ambiguous_author_with_nameblocking_zh) using pid_ao
-- Note denoting Chinese AND dataset
where CHAND > 0 and train1_test0_val2 in (0) -- Note eval on test set
order by xmol_clean_author_name;
'''
...
for feature_group_as_a_method, feature_group in feature_group_dict.items():
    ...
    ground_truths_ds, feature_and_metadata_ds = calc_groundtruth_predictions_given_dataset(df_block_test, model, feature_selector)
