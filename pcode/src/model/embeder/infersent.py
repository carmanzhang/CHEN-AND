...
from myconfig import glove840b300d_path, fasttextcrawl300d2m_path, infersent_based_path
...
class InferSentModel():
    def __init__(self, model_version, use_cuda=True):
    ...
    def infer_embedding(self, content_list, bsize=256):
        if self.model is None:
            self._load_model()
        return self.model.encode(content_list, bsize=bsize, tokenize=False)


if __name__ == '__main__':
    infsent = InferSentModel(model_version=1)
    embd = infsent.infer_embedding(['hello'])
    print(embd)
